...
class Player(Entity):
	def __init__(self,pos,groups,obstacle_sprites,create_attack,destroy_attack,create_magic, useKey):
		super().__init__(groups)
		self.image = pygame.image.load(IMAGES + 'player/down/down_0.png').convert_alpha()
	...
	def input(self):
		if not self.attacking:
			...
			if keys[pygame.K_3] and self.potionNum > 0 and self.can_drink:
				self.potionNum -= 1
				self.health += 20
				self.can_drink = False
				self.drink_time = pygame.time.get_ticks()
			
			if keys[pygame.K_4] and self.hasKey:
				self.useKey()
	...
	def cooldowns(self):
		...
		if not self.can_drink:
			if current_time - self.drink_time >= self.switch_duration_cooldown:
				self.can_drink = True
